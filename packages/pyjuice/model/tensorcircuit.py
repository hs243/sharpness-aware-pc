...
class TensorCircuit(nn.Module):
    ...
    def mini_batch_em(self, step_size: float, pseudocount: float = 0.0, keep_zero_params: bool = False,
                      step_size_rescaling: bool = False, use_cudagraph: bool = True, reg_lam: float=1.0, reg_mu: float=0.0,eta:float=None,phi:str=None):
        """
                    ...
                    for _,layer in enumerate(self.layers()):
                        if(isinstance(layer,SumLayer)):
                            for n in layer.nodes:
                                log_S = self.get_node_mars(n)
                                log_P = self.get_node_mars(n.chs[0])
                                log_result = (2*log_P.unsqueeze(0)) - (2*log_S.unsqueeze(1))
                                traces = torch.exp(log_result).sum(dim=2).sum(dim=1,keepdim=True) #absolute traces

                                pfsid, pfeid = n._param_flow_range
                                fpn = int((pfeid-pfsid)/n.num_nodes) #number of flows per node

                                reg_mu_t = torch.where(traces>eta,reg_mu,0)
                                reg_mu_t = torch.repeat_interleave(input=reg_mu_t,repeats=fpn)

                                self.param_flows[pfsid:pfeid] =(self.param_flows[pfsid:pfeid] + torch.sqrt(self.param_flows[pfsid:pfeid]**2 + 4*reg_lam*reg_mu_t*self.param_flows[pfsid:pfeid]))/2*reg_lam

            elif type(reg_mu)==torch.Tensor or reg_mu != 0.0:
                if(phi==None):
                    self.param_flows = (self.param_flows + torch.sqrt(self.param_flows**2 + 4*reg_lam*reg_mu*self.param_flows))/2*reg_lam
                elif(phi=="laplacian"):
                    phi_val = (self.param_flows/(self.param_flows+pseudocount)-reg_lam)/reg_mu
                    self.param_flows = self.param_flows/(reg_lam+reg_mu*phi_val)
                elif(phi=="entropy"):
                    phi_val = (1+torch.log(self.params[self.num_dummy_params:]))
                    self.param_flows = self.param_flows/(reg_lam+reg_mu*phi_val)
                elif(phi=="global_trace"):
                    phi_val = (torch.sqrt(self.param_flows**2 + 4*reg_lam*reg_mu*self.param_flows) - self.param_flows + 2*reg_lam*reg_mu)/(2*reg_mu**2)
                    self.param_flows = self.param_flows/(reg_lam+reg_mu*phi_val)
                elif(phi=="local_trace"):
                    with torch.no_grad():
                        for _,layer in enumerate(self.layers()):
                            if(isinstance(layer,SumLayer)):
                                for n in layer.nodes:
                                    log_S = self.get_node_mars(n)
                                    log_P = self.get_node_mars(n.chs[0])
                                    log_ratio = (2*log_P.unsqueeze(0)) - (2*log_S.unsqueeze(1))
                                    phi_val = torch.exp(log_ratio).mean(dim=2).reshape(-1) #mean along batch
                                    pfsid, pfeid = n._param_flow_range
                                    self.param_flows[pfsid:pfeid] = self.param_flows[pfsid:pfeid]/(reg_lam+reg_mu*phi_val)
